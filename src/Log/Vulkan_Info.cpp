...
// Vulkan Headers Version
  ...
  // Vulkan Version
  {
    ...
    std::string_view vk_version {
        std::to_string (VK_VERSION_MAJOR (PhysicalDeviceProperties.properties.apiVersion)) + '.'
          + std::to_string (VK_VERSION_MINOR (PhysicalDeviceProperties.properties.apiVersion)) + '.'
          + std::to_string (VK_VERSION_PATCH (PhysicalDeviceProperties.properties.apiVersion)) + '.'
          + std::to_string (VK_API_VERSION_VARIANT (PhysicalDeviceProperties.properties.apiVersion))
    };
  ...
  fast_io::io::println(
        ...
        PATATA_TERM_DIM, PATATA_TERM_COLOR_GRAY0,
                        ...
                        "  [",
                        std::string_view{ abi::__cxa_demangle (
                            typeid (Vulkan.SwapChain).name (), nullptr,
                            nullptr, nullptr) },
                        "] ",
#else
                        "  [",
                        std::string_view{
                            typeid (Vulkan.SwapChain).name () },
                        "] ",
#endif
#else
        "  ",
...
#if defined(__GNUC__) || defined(__MINGW64__) && !defined(__clang__)
      "    [",
      std::string_view{ abi::__cxa_demangle (typeid (SwapChainInfo.PresentMode).name (),
