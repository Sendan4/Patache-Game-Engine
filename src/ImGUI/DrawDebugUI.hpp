...
namespace Patata
{
        ...
        vk::Extent2D & SwapChainExtent)
    {
      ...
      ImGui::GetStyle ().Colors[ImGuiCol_Button] = ImVec4 (PATATA_IMGUI_PATATA_COLOR);
      ImGui::GetStyle ().Colors[ImGuiCol_ButtonHovered] = ImVec4 (PATATA_IMGUI_PATATA_SELECTED_COLOR);
      ImGui::GetStyle ().Colors[ImGuiCol_ButtonActive] = ImVec4 (PATATA_IMGUI_PATATA_DARKER_COLOR);
      ...
      if (pPatataEngineInfo->ShowMainMenuBar) {
          ...
          if (ImGui::Button("Info")) {
              if (pPatataEngineInfo->PatataInfoWindow) pPatataEngineInfo->PatataInfoWindow = false;
              else pPatataEngineInfo->PatataInfoWindow = true;
          }

          if (ImGui::Button("Config")) {
              if (pPatataEngineInfo->PatataConfigWindow) pPatataEngineInfo->PatataConfigWindow = false;
              else pPatataEngineInfo->PatataConfigWindow = true;
          }
    ...
    // Git Stage
        ...
        && defined(PATATA_GIT_WORK_DIR_IS_STAGED_BOOL)
                          ...
                          ImGui::TextColored (
                              ImVec4 (PATATA_IMGUI_POSITIVE_VALUE), "Clean");
#else
                          ImGui::TextColored (
                              ImVec4 (PATATA_IMGUI_WARNING_VALUE), "Dirty");
#endif

#if PATATA_GIT_WORK_DIR_IS_STAGED_BOOL == 1
                          ImGui::TextColored (
                              ImVec4 (PATATA_IMGUI_POSITIVE_VALUE),
                              "In Stage Area");
#else
                          ImGui::TextColored (
    ...
    // Game Name
                  ...
                  if (ImGui::TreeNode ("Libraries in use"))
                    {
                      if (ImGui::BeginTable ("Libraries", 2,
                                             PATATA_IMGUI_TABLE_FLAGS))
                        {
                          ImGui::TableSetupColumn (
    ...
    // Desktop
                          ...
                          if (pPatataEngineInfo->WindowInfo.subsystem
                                  == SDL_SYSWM_X11
                              && strcmp(pPatataEngineInfo->SessionType, "wayland") == 0)
                            ImGui::Text ("XWayland");
                          else if (pPatataEngineInfo->WindowInfo.subsystem
                                   == SDL_SYSWM_WAYLAND)
                            ImGui::Text ("Wayland");
                                             ...
                                             PATATA_IMGUI_TABLE_FLAGS))
                        {
                          ...
                          if (pPatataEngineInfo->WindowInfo.subsystem
                              == SDL_SYSWM_WAYLAND)
                            {
                              ...
                              if (pPatataEngineInfo->WindowInfo.info.wl
                                      .xdg_surface
                                  != nullptr)
                                {
                                  ImGui::TableNextRow ();
                                   ...
                                   == SDL_SYSWM_X11)
                            {
                              ...
                              // X11 / Xorg
                              // Has Display?
                              if (pPatataEngineInfo->WindowInfo.info.x11
                                      .display
                                  != nullptr)
                                {
                                  ImGui::TableNextRow ();
    ...
    // Has Window?
                          ...
                          if (pPatataEngineInfo->WindowInfo.subsystem
                              == SDL_SYSWM_WINDOWS)
                            {
                              ...
                              if (pPatataEngineInfo->WindowInfo.info.win
                                      .hinstance
                                  != nullptr)
                                {
                                  ImGui::TableNextRow ();
    ...
    // VVL Version
                  ...
                  if (ImGui::TreeNode ("Extensions and layers"))
                    {
                            ...
                            if (ImGui::BeginTable("Layers##VkLayerList", 1, PATATA_IMGUI_TABLE_FLAGS)) {
                                ImGui::TableSetupColumn (
                                    "Layer", ImGuiTableColumnFlags_WidthFixed);
                      ...
                      if (ImGui::BeginTable ("Instance##VkInstanceList", 1,
                        ...
                        {
                          ImGui::TableSetupColumn (
                              "Instance", ImGuiTableColumnFlags_WidthFixed);
                          ImGui::TableHeadersRow ();

                          for (uint16_t i = 0; i < pPatataEngineInfo->VkInstanceExtensionsCount; ++i)
                            {
                              ImGui::TableNextRow ();
                      ...
                      if (ImGui::BeginTable ("Device##VkDeviceList", 1,
                        ...
                        {
                          ImGui::TableSetupColumn (
                              "Device", ImGuiTableColumnFlags_WidthFixed);
                          ImGui::TableHeadersRow ();

                          for (uint32_t i = 0;
                               i < pPatataEngineInfo->VkDeviceExtensionsCount;
                               ++i)
                            {
                              ImGui::TableNextRow ();
                          ...
                          if (pPatataEngineInfo->VkDeviceVendorId != 0)
                            {
                              ...
                              switch (pPatataEngineInfo->VkDeviceVendorId)
                                ...
                                case 32902:
                                  ImGui::TextColored (
                                      ImVec4 (PATATA_IMGUI_INTEL_COLOR),
                                      "Intel");
                                  break;
                                case 4098:
                                  ImGui::TextColored (
                                      ImVec4 (PATATA_IMGUI_AMD_COLOR), "AMD");
                                  break;
                                case 4318:
                                  ImGui::TextColored (
                                      ImVec4 (PATATA_IMGUI_NVIDIA_COLOR),
                                      "Nvidia");
                                  break;
                                case 65541:
                                  ImGui::TextColored (
                                      ImVec4 (PATATA_IMGUI_MESA_COLOR),
                                      "Mesa");
                                  break;
                                default:
                                  ImGui::TextColored (
                                      ImVec4 (
                                          PATATA_IMGUI_UNKNOWN_VENDOR_COLOR),
                                      "Unknown");
                                  break;
                                }
                  ...
                  if (ImGui::TreeNode ("Driver"))
                    {
                      if (!pPatataEngineInfo->VkDriverName.empty())
                        {
                          ImGui::Text (
                              "%s", pPatataEngineInfo->VkDriverName.c_str ());
                        }
                          ...
                          if (!pPatataEngineInfo->VkDriverVersion.empty())
                            {
                              ...
                              ImGui::Text (
                                  "%s",
                                  pPatataEngineInfo->VkDriverVersion.c_str ());
                            }
                  ...
                  if (ImGui::TreeNode ("SwapChain"))
                    {
                      if (ImGui::BeginTable ("Vulkan SwapChain Info##1", 2,
                                             PATATA_IMGUI_TABLE_FLAGS))
                        {
                          ...
                          ImGui::Text (
                              "%s",
                              vk::to_string (pPatataEngineInfo
                                                 ->VkSwapchainImageColorFormat)
                                  .c_str ());
      ...
      if (pPatataEngineInfo->PatataConfigWindow)
      {
          ImGui::Begin ("Configuration", &pPatataEngineInfo->PatataConfigWindow);
...
#if defined(__linux__)
              if (ImGui::Checkbox ("Prefer Wayland",
                                   &pConfiguration->PreferWayland))
                {
                  pConfiguration->TriggeredChange
                      = &pConfiguration->PreferWayland;
                }

              if (ImGui::Checkbox ("MangoHud",
                                   &pConfiguration->EnableMangoHud))
                {
                  pConfiguration->TriggeredChange
                      = &pConfiguration->EnableMangoHud;
                }
              ...
              if (ImGui::Checkbox ("10 Bit Depth",
                                   &pConfiguration->BitDepth10))
                {
                  pConfiguration->TriggeredChange
                      = &pConfiguration->BitDepth10;
                }
