...
Patata::RaccoonRenderer::InitVulkanBackend (void)
{
  ...
  */
  ...
  #if defined(DEBUG)
  std::future<bool> CreateImguiDescriptorPool_Async = std::async(std::launch::async,
      &Patata::RaccoonRenderer::CreateImguiDescriptorPool, this);
  std::future<bool> CreateImguiPipelineCache_Async = std::async(std::launch::async,
      &Patata::RaccoonRenderer::CreateImguiPipelineCache, this);
  #endif

  // Create a surface for the window to draw on
  if (!SDL_Vulkan_CreateSurface (*pRaccoonInfo->ppWindow, Vulkan.Instance,
  ...
  if (!CreateSwapChain (SwapChainInfo)) return false;

  if (!CreateImageView (SwapChainInfo)) return false;
  ...
  std::future<bool> CreateFence_Async = std::async(std::launch::async,
      &Patata::RaccoonRenderer::CreateFence, this);
  ...
  std::future<void> VulkanInfo_Async = std::async (std::launch::async,
      &Patata::RaccoonRenderer::VulkanInfo, this, SwapChainInfo);

  #if defined(DEBUG)
  CreateImguiPipelineCache_Async.wait();
  if (!CreateImguiPipelineCache_Async.get()) return false;
  CreateImguiDescriptorPool_Async.wait();
  if (!CreateImguiDescriptorPool_Async.get()) return false;
  ...
  */
  ...
  if (!CreateFrameBuffer_Async.get()) return false;
  CreateCommandBuffer_Async.wait();
  if (!CreateCommandBuffer_Async.get()) return false;
  CreateFence_Async.wait();
  if (!CreateFence_Async.get()) return false;
  CreateSemaphores_Async.wait();
  if (!CreateSemaphores_Async.get()) return false;
