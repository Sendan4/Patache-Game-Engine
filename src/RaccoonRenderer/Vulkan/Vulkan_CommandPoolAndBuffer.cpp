...
Patata::RaccoonRenderer::CreateCommandPool (void) {
    ...
    vk::Result Result = Vulkan.Device.createCommandPool (&CommandPoolInfo,
                                                  nullptr, &Vulkan.CommandPool);
    if (Result != vk::Result::eSuccess) {
      std::future<void> ReturnVulkanCheck = std::async (std::launch::async,
          Patata::Log::VulkanCheck, "Command Pool", Result);
      return false;
    }
    return true;
}
...
Patata::RaccoonRenderer::CreateCommandBuffer (void)
{
    ...
    if (Result != vk::Result::eSuccess) {
        std::future<void> ReturnVulkanCheck = std::async (std::launch::async,
            Patata::Log::VulkanCheck, "Allocate Command Buffer", Result);
