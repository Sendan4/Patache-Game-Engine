...
CreateSwapchain (Patache::Engine * const pEngine, Patache::SwapchainInfo & rSwapchainInfo)
{
  ...
  if (result != vk::Result::eSuccess)
    {
      std::future<void> returnVulkanCheck
          = std::async (std::launch::async, Patache::VulkanCheck, "Get Surface Formats", result);
    }
  ...
  */
  ...
  if (result != vk::Result::eSuccess)
    {
      ...
      std::future<void> returnVulkanErr = std::async (
          std::launch::async, Patache::FatalErrorMessage, "Patache pEngine - Raccoon Renderer",
  ...
  if (result != vk::Result::eSuccess)
    {
      std::future<void> returnVulkanCheck
          = std::async (std::launch::async, Patache::VulkanCheck, "Swapchain", result);
