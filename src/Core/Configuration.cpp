  ...
  if (YamlConfig.invalid () || !YamlConfig.readable ())
    {
      std::future<void> Err = std::async (
          std::launch::async, Patache::Log::FatalErrorMessage, "Patache Engine - Yaml",
          "Yaml Config File is invalid or inreadable", configuration);
  ...
  else
    {
      ...
      if (YamlConfig["patache-engine"]["show-fatal-error-messagebox"].val () == "true"
      ...
      else
        {
          ...
          std::future<void> Err
              = std::async (std::launch::async, Patache::Log::ErrorMessage, ErrorText);
        }
      ...
      if (YamlConfig["patache-engine"]["raccoon-renderer"]["vsync"].val () == "true"
        ...
        {
          YamlConfig["patache-engine"]["raccoon-renderer"]["vsync"] >> configuration.Vsync;
        }
      else
        {
          char ErrorText[PATACHE_ERROR_TEXT_SIZE]{ 0 };
